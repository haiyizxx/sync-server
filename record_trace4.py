...
def continuous_recording(mc, trace, start_time):
    ...
    # Check gripper availability once at the start
    gripper_available = hasattr(mc, 'get_gripper_value')
    print(f"Gripper functions available: {gripper_available}")
    
    # Test gripper read once at start
    if gripper_available:
        try:
            test_value = mc.get_gripper_value()
            print(f"Initial gripper test value: {test_value}")
        except Exception as e:
            print(f"Initial gripper test failed: {e}")
            gripper_available = False
    
    recording_count = 0
    gripper_read_count = 0
    ...
    while recording:
        try:
            ...
            if angles is not None:
                ...
                # Get gripper value with error handling
                gripper_value = None
                if gripper_available:
                    try:
                        gripper_value = mc.get_gripper_value()
                        gripper_read_count += 1
                    except Exception as e:
                        if recording_count % 100 == 0:  # Only print error every 100 attempts
                ...
                if recording_count % 50 == 0:
                    gripper_info = f"gripper: {gripper_value}" if gripper_value is not None else "gripper: N/A"
                    print(f"Recorded {recording_count} points... {gripper_info} (gripper reads: {gripper_read_count})")
    ...
    print(f"Recording thread stopped. Total recorded: {recording_count}, Gripper reads: {gripper_read_count}")
