#!/usr/bin/env python3
"""
Script to match trace timestamps with image timestamps for any episode.
Fills the 'image' field in each trace entry with the best matching image filename.

Usage:
    python3 match_traces_images_flexible.py <episode_name> [--tolerance <ms>] [--overwrite-original]
    
Examples:
    python3 match_traces_images_flexible.py e3
    python3 match_traces_images_flexible.py e3 --tolerance 300
    python3 match_traces_images_flexible.py e3 --overwrite-original
"""
...
def load_image_timestamps(images_dir: str) -> Dict[int, str]:
    ...
    for json_file in json_files:
        try:
            ...
            # Convert timestamp from seconds (with decimals) to milliseconds
...
def match_traces_with_images(episode_name: str, max_tolerance_ms: int = 200, overwrite_original: bool = False):
    """
    Main function to match traces with images for a given episode.
    """
    # Paths
    trace_file = f"traces/{episode_name}.json"
    images_dir = f"images/{episode_name}"
    
    if overwrite_original:
        output_file = trace_file
    else:
        output_file = f"traces/{episode_name}_matched.json"
    ...
    for timestamp_ms, filename in sorted(image_timestamps.items()):
        print(f"  {timestamp_ms} ms -> {filename}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Match trace timestamps with image timestamps",
...
Examples:
  ...
  python3 match_traces_images_flexible.py e3 --overwrite-original
    ...
    parser.add_argument(
        "episode_name",
        help="Name of the episode to process (e.g., e3)"
    )
    ...
    try:
        ...
        if trace_data is not None:
            print(f"\n✅ Successfully processed episode {args.episode_name}")
            sys.exit(0)
        else:
            print(f"\n❌ Failed to process episode {args.episode_name}")
            sys.exit(1)
            
    except Exception as e:
        print(f"❌ Error processing episode {args.episode_name}: {e}")
        sys.exit(1)
